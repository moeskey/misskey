...
export const meta = {
    ...
    errors: {
        mtlDisabled: {
            message: 'Mutual timeline has been disabled.',
            code: 'MTL_DISABLED',
            id: '0bc82de5-aa13-0208-d40e-b6a14edaa03a',
        },

        bothWithRepliesAndWithFiles: {
            message: 'Specifying both withReplies and withFiles is not supported',
            code: 'BOTH_WITH_REPLIES_AND_WITH_FILES',
            id: '7ade1192-e7fc-5bf8-edae-82773e7d0944',
        },
	},
} as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
    ...
    private async getFromDb(ps: { untilId: string | null; sinceId: string | null; limit: number; includeMyRenotes: boolean; includeRenotedMyNotes: boolean; includeLocalRenotes: boolean; withFiles: boolean; withRenotes: boolean; withReplies: boolean; }, me: MiLocalUser) {
        const mutualIds = await this.userFollowingService.getMutualFolloweeIds(me.id);
        ...
        if (mutualIds.length > 0 && followingChannelIds.length > 0) {
            // ユーザー・チャンネルともにフォローあり
            const meOrFolloweeIds = [me.id, ...mutualIds];
